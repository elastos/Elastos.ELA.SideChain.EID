...
//   reserving it for Elastos.ELA.SideChain.EID. This would also reduce the memory requirements
...
func (f *freezer) freeze(db ethdb.KeyValueStore, config *params.ChainConfig) {
	...
	for {
		...
		for i := 0; i < len(ancients); i++ {
			...
			if first+uint64(i) != 0 {
				DeleteBlockWithoutNumber(nfdb, ancients[i], first+uint64(i), config, true)
		...
		for number := first; number < f.frozen; number++ {
			...
			if number != 0 {
				for _, hash := range ReadAllHashes(db, number) {
					DeleteBlock(db, hash, number, config, true)
				}
			}
		}
		if err := batch.Write(); err != nil {
