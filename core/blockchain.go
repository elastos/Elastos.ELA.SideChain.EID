...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	delFn := func(db ethdb.KeyValueWriter, hash common.Hash, num uint64) {
		...
		if num+1 <= frozen {
			...
			rawdb.DeleteReceipts(bc.db, hash, num, bc.chainConfig)
		}
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
			...
			for {
				...
				if len(deleted) > 100000 {
					...
					for _, nh := range deleted {
						rawdb.DeleteBlockWithoutNumber(bc.db, nh.hash, nh.number, bc.chainConfig)
					...
					for _, nh := range deleted {
						for _, hash := range rawdb.ReadAllHashes(bc.db, nh.number) {
							rawdb.DeleteBlock(bc.db, hash, nh.number, bc.chainConfig)
						}
					}
					if err := batch.Write(); err != nil {
						return 0, err
		...
		for _, block := range blockChain {
			...
			if block.NumberU64() != 0 {
				rawdb.DeleteBlockWithoutNumber(bc.db, block.Hash(), block.NumberU64(), bc.chainConfig)
		...
		for _, block := range blockChain {
			...
			if block.NumberU64() != 0 {
				for _, hash := range rawdb.ReadAllHashes(bc.db, block.NumberU64()) {
					rawdb.DeleteBlock(bc.db, hash, block.NumberU64(), bc.chainConfig)
				}
			}
		}
		if err := batch.Write(); err != nil {
