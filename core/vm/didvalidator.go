...
func checkDIDOperation(evm *EVM, header *did.Header,
	...
	if dbExist {
		if header.Operation == did.Create_DID_Operation && !evm.Config.Debug {
			...
			configHeight := evm.chainConfig.OldDIDMigrateHeight
			if evm.Context.BlockNumber.Cmp(configHeight) > 0 && !evm.Config.Debug {
...
func checkTicketAvailable(evm *EVM, cPayload *did.DIDPayload,
	...
	// check transactionID
	if lastTxHash[:2] == "0x" {
		lastTxHash = lastTxHash[2:]
	}
	ticketTXID := cPayload.Ticket.TransactionID
	if ticketTXID[:2] == "0x" {
		ticketTXID = ticketTXID[2:]
	}
	if ticketTXID != lastTxHash {
		return errors.New(fmt.Sprintf("invalid TransactionID of ticket ticket.txid:%s, lastTxHash:%s", cPayload.Ticket.TransactionID, lastTxHash))
	}
...
func checkCustomizedDIDOperation(evm *EVM, header *did.Header,
	...
	if dbExist {
		if header.Operation == did.Create_DID_Operation {
			...
			if evm.Context.BlockNumber.Cmp(targetHeight) < 0 && !evm.Config.Debug {
