...
func New(ctx *node.ServiceContext, config *Config, node *node.Node) (*Ethereum, error) {
	...
	chainDb, err := ctx.OpenDatabaseWithFreezer("chaindata", config.DatabaseCache, config.DatabaseHandles, config.DatabaseFreezer, "eth/db/chaindata/", config.Genesis.Config)
	if err != nil {
		return nil, err
	}
